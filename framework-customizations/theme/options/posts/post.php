<?php if ( ! defined( 'FW' ) ) {
	die( 'Forbidden' );
}

$options = array(
	'post-layout-section'         => array(
		'title'   => esc_html__( 'Featured Image Position', 'umodel' ),
		'type'    => 'box',
		'context' => 'side',
		'options' => array(
			'post-layout' => array(
				'type'    => 'image-picker',
				'value'   => 'post-layout-standard',
				'label'   => esc_html__( 'Post layout in blog feed', 'umodel' ),
				'desc'    => esc_html__( 'Position of image in blog feed', 'umodel' ),
				'help'    => esc_html__( 'Standard position of image at the top of the post. Or small image at the left of the post. Works only if featured image chosen.', 'umodel' ),
				'choices' => array(
					'post-layout-standard' => UMODEL_THEME_URI . '/img/theme-options/post-layout-standard.png',
					'post-layout-small'    => UMODEL_THEME_URI . '/img/theme-options/post-layout-small.png',
				),
			),
			'show-readmore' => array(
				'type'         => 'switch',
				'label'        => esc_html__( 'Post read more link', 'umodel' ),
				'value'        => ' ',
				'left-choice'  => array(
					'value' => 'readmore-hidden',
					'label' => esc_html__( 'Hide', 'umodel' ),
				),
				'right-choice' => array(
					'value' => '',
					'label' => esc_html__( 'Show', 'umodel' ),
				),
			),
		),
	),
	'post-featured-video-section' => array(
		'title'   => esc_html__( 'Post Featured Video', 'umodel' ),
		'type'    => 'box',
		'context' => 'side',

		'options' => array(
			'post-featured-video' => array(
				'type'    => 'oembed',
				'value'   => '',
				'label'   => esc_html__( 'Featured Video', 'umodel' ),
				'desc'    => esc_html__( 'Adds featured video', 'umodel' ),
				'help'    => esc_html__( 'Leave blank if no needed', 'umodel' ),
				'preview' => array(
					'width'      => 278, // optional, if you want to set the fixed width to iframe
					'height'     => 185, // optional, if you want to set the fixed height to iframe
					/**
					 * if is set to false it will force to fit the dimensions,
					 * because some widgets return iframe with aspect ratio and ignore applied dimensions
					 */
					'keep_ratio' => true
				),
			),
		),
	),
    'custom-class-section'         => array(
        'title'   => esc_html__( 'Custom class', 'umodel' ),
        'type'    => 'box',
        'context' => 'side',
        'options' => array(
            'post-layout' => array(
                'type'    => 'text',
                'value'   => '',
                'attr'  => array( 'class' => 'custom-class', 'data-foo' => 'bar' ),
                'label'   => esc_html__( 'custom class', 'umodel' ),
                'desc'    => esc_html__( 'custom class', 'umodel' ),
                'help'    => esc_html__( 'custom class', 'umodel' ),
                'add-button-text' => esc_html__('Add', '{domain}'),
            ),
        ),
    ),
);
