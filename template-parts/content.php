<?php
/**
 * The default template for displaying content
 *
 * Used for both single and index/archive/search.
 */

if ( ! defined( 'ABSPATH' ) ) {
	exit; // Exit if accessed directly
}

$show_post_thumbnail = ( post_password_required() || is_attachment() || ! has_post_thumbnail() ) ? false : true;

$post_categories = function_exists( 'fw_get_db_customizer_option' ) ? fw_get_db_customizer_option( 'post_categories' ) : 'yes';
$post_author     = function_exists( 'fw_get_db_customizer_option' ) ? fw_get_db_customizer_option( 'post_author' ) : 'yes';
$post_date       = function_exists( 'fw_get_db_customizer_option' ) ? fw_get_db_customizer_option( 'post_date' ) : 'yes';
$post_tags       = function_exists( 'fw_get_db_customizer_option' ) ? fw_get_db_customizer_option( 'post_tags' ) : 'yes';

$post_layout = 'post-layout-standard';
if ( function_exists( 'fw_get_db_post_option' ) ) {
	$post_layout = fw_get_db_post_option( $post->ID, 'post-layout', 'post-layout-standard' );
}
$show_readmore = function_exists( 'fw_get_db_post_option' ) ? fw_get_db_post_option( $post->ID, 'show-readmore', 'readmore-hidden' ) : '';
//standard feed layout (image at the top or not image at all if option is standard or has no post thumbnail)
$small_layout = ( $post_layout == 'post-layout-standard' || ! $show_post_thumbnail ) ? false : true;

$read_more_add_class = '';
if (function_exists( 'mwt_share_this')) {
	$read_more_add_class = 'absolute_button';
}

if ( !is_singular() && $small_layout ) : //additional markup for small layout post
?>
<article id="post-<?php the_ID(); ?>" <?php post_class( 'format-small-image ' . $show_readmore . ' ' . $read_more_add_class  ); ?>>
    <div class="side-item content-padding big-padding with_background overflow-hidden">
        <div class="row">
			<?php umodel_post_thumbnail( $small_layout ); ?>
            <div class="col-md-6">

				<?php else : //standard layout markup  ?>

                <article
                        id="post-<?php the_ID(); ?>" <?php post_class( 'vertical-item ' . $show_readmore . ' ' . $read_more_add_class  ); ?>>
					<?php
					umodel_post_thumbnail();
					endif; //small_format check
					?>

                    <div class="item-content">

                        <div class="entry-header flex-wrap v-center justify-sb">
                            <div class="left-part flex-wrap v-center">
                                <?php if ( ! defined( 'FW' ) ) : ?>
                                    <div class="entry-author">
                                        <?php if ( 'post' == get_post_type() ) {
                                            umodel_posted_by();
                                        } ?>
                                    </div>
                                <?php elseif ( defined( 'FW' ) && $post_author == 'yes' ) :  ?>
                                    <div class="entry-author">
                                        <?php if ( 'post' == get_post_type() ) {
                                            umodel_posted_by();
                                        } ?>
                                    </div>
                                <?php endif; ?><!-- .entry-author -->
                                <?php if ( ! defined( 'FW' ) ) : ?>
                                    <div class="categories-links color2">
                                        <?php
                                        echo '<span class="separator">/</span>';
                                        echo get_the_category_list( _x( ' ', 'Used between list items, there is a space after the comma.', 'umodel' ) );
                                        ?>
                                    </div>
                                <?php elseif ( defined( 'FW' ) && $post_categories == 'yes' ) :  ?>
                                    <?php if ( in_array( 'category', get_object_taxonomies( get_post_type() ) ) && umodel_categorized_blog() ) : ?>
                                        <div class="categories-links color2">
                                            <?php
                                            echo '<span class="separator">/</span>';
                                            echo get_the_category_list( _x( ', ', 'Used between list items, there is a space after the comma.', 'umodel' ) );
                                            ?>
                                        </div>
                                    <?php endif; ?>
                                <?php endif; ?>
                                <!-- .item cats -->
                            </div>
                            <div class="right-part">
                                <?php if ( ! defined( 'FW' ) ) : ?>
                                    <div class="entry-date">
                                        <?php if ( 'post' == get_post_type() ) {
                                            umodel_posted_on();
                                        } ?>
                                    </div>
                                <?php elseif ( defined( 'FW' ) && $post_date == 'yes' ) :  ?>
                                    <div class="entry-date">
                                        <?php if ( 'post' == get_post_type() ) {
                                            umodel_posted_on();
                                        } ?>
                                    </div>
                                <?php endif; ?>
                                <!-- .entry-date -->
                            </div>
                        </div><!-- .entry-header -->

	                    <?php
	                    the_title( '<h4 class="entry-title"><a href="' . esc_url( get_permalink() ) . '" rel="bookmark">', '</a></h4>' );
	                    ?>

						<?php if ( is_search() ) : ?>
                            <div class="entry-summary">
								<?php the_excerpt(); ?>
                            </div><!-- .entry-summary -->
						<?php else : ?>
                            <div class="entry-content">
								<?php
								//hidding "more link" in content
								the_content( esc_html__( 'Read more', 'umodel' ) );
								?>
	                            <?php
	                            wp_link_pages( array(
		                            'before'      => '<div class="page-links topmargin_30"><span class="page-links-title">' . esc_html__( 'Pages:', 'umodel' ) . '</span>',
		                            'after'       => '</div>',
		                            'link_before' => '<span>',
		                            'link_after'  => '</span>',
	                            ) );
	                            ?>
                                <div class="entry-footer">
	                                <?php if(is_singular()) :
                                    if ( ! defined( 'FW' ) ) : ?>
                                        <div class="entry-tags">
			                                <?php the_tags( '<span class="tag-links">', ' ', '</span>' ); ?>
                                        </div>
	                                <?php elseif ( defined( 'FW' ) && $post_tags == 'yes' ) :  ?>
                                        <div class="entry-tags">
			                                <?php the_tags( '<span class="tag-links">', ' ', '</span>' ); ?>
                                        </div>
	                                <?php endif; endif; ?>
                                    <!-- .item tags -->

                                    <?php umodel_blog_adds() ?>
                                    <!-- eof .blog-adds -->
                                </div>

                            </div><!-- .entry-content -->
						<?php endif; //is_search
						?>
                    </div><!-- eof .item-content -->
					<?php if ( !is_singular() && $small_layout ) : //additional markup for small format post   ?>
            </div><!-- eof .col-md-6 -->
        </div><!-- eof .row -->
    </div><!-- eof .side-item -->
	<?php endif; //small_format
	?>
</article><!-- #post-## -->