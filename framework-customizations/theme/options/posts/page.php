...
$options = array(
	'page-options-section' => array(
		'title'   => esc_html__( 'Additional Options', 'umodel' ),
		'type'    => 'box',
		'context' => 'normal',
		'options' => array(
		),
...
if ( ! empty( $choices ) ) {
	...
	$choices['0'] = esc_html__( 'No Slider', 'umodel' );
	...
	array_push( $options['page-options-section']['options'], array(
			...
			'no-forms'  => array(
				...
				'html'  =>
					'<div>' .
					'<h1 style="font-weight:100; text-align:center;">' . esc_html__( 'No Sliders Available', 'umodel' ) . '</h1>' .
					...
					str_replace(
						...
						array(
							...
							fw_html_tag( 'a', array(
								...
								'target' => '_blank',
							), esc_html__( 'create a new Slider', 'umodel' ) )
						),
						esc_html__( 'No Sliders created yet. Please go to the {br}Sliders page and {add_slider_link}.', 'umodel' )
					) .
					'</em>' .
					'</p>' .
					'</div>'
